...
func (k8scm *K8sCMCache) Create(identifier string, data interface{}) error {
	...
	if cm != nil && err == nil {
		glog.Infof("k8s-cm-cache: configmap %s already exists, skipping configmap creation", identifier)
		return nil
	}
	...
	glog.Infof("k8s-cm-cache: configmap %s successfully created", identifier)
	return nil
}
